...
def contar_frecuencia_palabras(texto):
    ...
    """
    # Limpiar el texto: encontrar todas las palabras
...
def analizar_texto_completo(texto):
    ...
    # 1. Analizar la estructura básica
    estadisticas = analizar_estructura(texto)
    print("\nEstadísticas del texto:")
    for clave, valor in estadisticas.items():
        print(f" - {clave}: {valor}")
    
    # 2. Contar la frecuencia de palabras (el núcleo de este ejercicio)
    frecuencias = contar_frecuencia_palabras(texto)
    
    # 3. Mostrar el reporte de frecuencias
